...
def _import_model(directed_test_yaml: pathlib.Path,
                  selected_tests: List[str] | None = None) -> dict:
    ...
    raw_tests = filter((lambda i: i.get('test') is not None), yaml_data)
    if selected_tests is not None:
        raw_tests = filter((lambda i: i.get('test') in selected_tests), raw_tests)
...
def import_model(directed_test_yaml: pathlib.Path) -> dict:
    return _import_model(directed_test_yaml)


def import_selected_model(directed_test_yaml: pathlib.Path,
                          selected_tests: List[str]) -> dict:
    return _import_model(directed_test_yaml, selected_tests)
