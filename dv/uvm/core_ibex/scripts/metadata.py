...
class RegressionMetadata(scripts_lib.testdata_cls):
    ...
    custom_test_data            : pathlib.Path = field(init=False, compare=False, default_factory=pathlib.Path)
    ...
    def __post_init__(self):
        ...
        self.custom_test_data            = self.directed_test_dir/'custom_testlist.yaml'
    ...
    def get_tests_and_counts(self) -> List[Tuple[Tuple[str, int, TestType], object]]:
        ...
        """
        test_selectors = self.test.split(',')
        explicit_directed = any(x in test_selectors
                                for x in ['all_directed', 'all_custom'])

        riscvdv_matched_list: ibex_cmd._TestEntries = self.process_riscvdv_testlist()
        # Do not validate directed testlists for a RISCV-DV-only selection. This
        # checkout intentionally removes some upstream directed/vendor trees, but
        # RISCV-DV random tests should remain runnable.
        directed_matched_list: ibex_cmd._TestEntries = (
            self.process_directed_testlist()
            if explicit_directed or not riscvdv_matched_list else [])
    ...
    def process_directed_testlist(self) -> ibex_cmd._TestEntries:
        """Extract test information from the directed test YAML files.
        ...
        matched_list: ibex_cmd._TestEntries = []
        test_selectors = self.test.split(',')
        selected_tests = [x for x in test_selectors
                          if x not in ['all', 'all_riscvdv',
                                       'all_directed', 'all_custom']]

        if self.custom_test_data.exists():
            custom_model = (
                directed_test_schema.import_model(self.custom_test_data)
                if any(x in test_selectors for x in ['all_directed', 'all_custom'])
                else directed_test_schema.import_selected_model(
                    self.custom_test_data, selected_tests))
            for entry in custom_model.get('tests'):
                select_test = any(x in test_selectors
                                  for x in ['all_directed', 'all_custom',
                                            entry.get('test')])
                if select_test:
            ...
            if 'all_custom' in test_selectors:
                return matched_list

        m = (directed_test_schema.import_model(self.directed_test_data)
             if 'all_directed' in test_selectors
             else directed_test_schema.import_selected_model(
                 self.directed_test_data, selected_tests))
