...
def reloc_word(simulator: str,
    ...
    sim_relocs = {
        ...
        'questa': [
            # Questa run command may refer to the generated TCL script
            # and/or the simulator build directory.
            (placeholder_dir, build_dir),
            (os.path.join(placeholder_dir, 'questa_sim.tcl'),
             os.path.join(build_dir, 'questa_sim.tcl')),
            (os.path.join(placeholder_dir, 'modelsim.ini'),
             os.path.join(build_dir, 'modelsim.ini')),
            (os.path.join(placeholder_dir, 'work'),
...
def _main() -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        run_failed = ret != 0
        do_file_copies(td, trr.dir_test, file_copies, run_failed)
        if run_failed:
