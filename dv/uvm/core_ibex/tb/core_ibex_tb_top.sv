...
`endif
  ...
  // --------------------------------------------------------------------------
  // Optional RVFI instruction print to Questa Transcript
  //
  // Enable with:
  // +print_rvfi_instr
  //
  // This prints every committed instruction observed from DUT RVFI.
  // --------------------------------------------------------------------------
  bit print_rvfi_instr;

  initial begin
  print_rvfi_instr = $test$plusargs("print_rvfi_instr");

  if (print_rvfi_instr) begin
    $display("[RVFI_PRINT] Enabled. Printing committed DUT instructions from RVFI.");
  end

  end

  always_ff @(posedge clk) begin
  if (print_rvfi_instr && rvfi_if.valid) begin
  $display("[RVFI] t=%0t order=%0d pc=%08h insn=%08h pc_next=%08h trap=%0b intr=%0b "
  // register source/destination
  // rs1/rs2 are the architectural source operands seen by RVFI
  // rd is the architectural destination writeback seen by RVFI
  //
  // Example:
  // insn=00100093 means raw 32-bit RISC-V instruction
  // pc = PC before executing instruction
  // pc_next = PC after executing instruction
  ,
  $time,
  rvfi_if.order,
  rvfi_if.pc_rdata,
  ...
  rvfi_if.intr);
    ...
    $display("       rs1=x%0d rs1_rdata=%08h rs2=x%0d rs2_rdata=%08h rd=x%0d rd_wdata=%08h",
            rvfi_if.rs1_addr,
            rvfi_if.rs1_rdata,
            rvfi_if.rs2_addr,
            rvfi_if.rs2_rdata,
    ...
    $display("       mem_addr=%08h mem_rmask=%02h mem_wmask=%02h mem_rdata=%08h mem_wdata=%08h",
            rvfi_if.mem_addr,
            rvfi_if.mem_rmask,
            rvfi_if.mem_wmask,
            rvfi_if.mem_rdata,
